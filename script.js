// State variables
let display = '', expression = '', history = [], currentSlide = 0;
...
function handleKeyboard(e) {
    if (e.key >= '0' && e.key <= '9') appendValue(e.key);
        ...
        el.historyModal.classList.toggle('show', showHistory);
        if (showHistory) {
            updateHistoryDisplay();
        }
    }
...
function clearAllHistory() {
    if (confirm('Apakah yakin ingin menghapus semua riwayat?')) {
        history = [];
        saveHistory();
        updateHistoryDisplay();
    }
